...
public class WithdrawnMapper {
    ...
    public Withdrawn mapToWithdrawnRequest(WithdrawnRequest request) {
        Withdrawn withdrawn = new Withdrawn() ;
        withdrawn.setAmount(request.getAmount());
        withdrawn.setStatus(request.getStatus());
        withdrawn.setMoneyRemaining(request.getMoneyRemaining());
        return withdrawn ;
    }
}
